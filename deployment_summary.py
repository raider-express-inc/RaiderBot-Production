            ...
            if os.path.exists(file_path):
                with open(file_path, "r") as f:
                    ...
                    if deployment_data.get("status") == "deployed":
                        ...
                        print(
                            f"✅ {deployment_data['component']}: {deployment_data['status']}"
                        )
                        print(
                            f"   Location: {deployment_data.get('deployment_location', 'N/A')}"
                        )
                        print(
                            f"   Integration: {deployment_data.get('integration_status', 'N/A')}"
                        )
                        print()

        print(
            f"📈 Deployment Success Rate: {successful_deployments}/{total_components} ({(successful_deployments / total_components) * 100:.1f}%)"
        )
