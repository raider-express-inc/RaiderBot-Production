...
class WorkbookInstructionService:
    ...
    async def push_visualization_instructions(
        self, instruction: VisualizationInstruction
    ) -> Dict[str, Any]:
        """Push visualization instructions to user workbook"""
        try:
            viz_config = self._generate_visualization_config(instruction)

            await self._push_to_workbook(instruction.workbook_id, viz_config)
    ...
    def _generate_visualization_config(
        ...
        if instruction.visualization_type == "chart":
            ...
            metrics_config = {
                ...
                "comparison": instruction.chart_config.get(
    ...
    async def _push_to_workbook(
        self, workbook_id: str, viz_config: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Push visualization config to Foundry workbook"""
        if self.foundry_client:
            return await self.foundry_client.update_workbook_visualization(
                workbook_id, viz_config
            )
        else:
            return {
                "status": "pushed",
    ...
    async def provision_user_dashboard(
        self, user_id: str, user_role: str, template: str = "default"
    ) -> Dict[str, Any]:
