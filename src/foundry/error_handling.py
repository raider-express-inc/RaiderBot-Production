...
from src.raiderbot_auth import PalantirAuthError, AuthenticationFailedError
...
class ErrorHandler:
    ...
    def log_error(self, error: Exception, context: Dict[str, Any] = None):
        ...
        emit_metric(
            ...
            1,
            dimensions={"error_type": error_type, "service": "raiderbot"},
        )
    ...
    def handle_http_error(
        self, status_code: int, response_text: str, url: str = ""
    ) -> Exception:
        ...
        if status_code == 401:
            ...
            error = PalantirDataError(
                f"Request failed ({status_code}): {response_text}"
            )
    ...
    def handle_ontology_error(self, error: Exception, operation: str) -> Exception:
        ...
        if "not found" in str(error).lower():
            return PalantirOntologyError(
                f"Ontology object not found during {operation}: {error}"
            )
        elif "permission" in str(error).lower():
            return PalantirResourceError(
                f"Insufficient permissions for {operation}: {error}"
            )
        else:
            return PalantirOntologyError(
                f"Ontology operation failed ({operation}): {error}"
            )
    ...
    def handle_workshop_error(self, error: Exception, operation: str) -> Exception:
        ...
        if "workspace" in str(error).lower():
            return PalantirWorkshopError(
                f"Workshop workspace error during {operation}: {error}"
            )
        elif "widget" in str(error).lower():
            return PalantirWorkshopError(
                f"Workshop widget error during {operation}: {error}"
            )
        else:
            return PalantirWorkshopError(
                f"Workshop operation failed ({operation}): {error}"
            )
