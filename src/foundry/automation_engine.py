...
from .workbook_instruction_service import (
    WorkbookInstructionService,
...
class RaiderBotAutomationEngine:
        ...
        if "tms" in request_lower:
            build_steps.append(
                BuildStep(
                    ...
                    config={"source": "snowflake", "tables": ["ORDERS", "CUSTOMERS"]},
                )
            )

        if any(
            keyword in request_lower
            for keyword in ["chart", "graph", "visualization", "graphics"]
        ):
            build_steps.append(
                BuildStep(
                    ...
                    config={
                        "visualization_type": self._detect_visualization_type(
                            request_lower
                        ),
    ...
    async def _execute_build_plan(
        self, build_plan: List[BuildStep], branch: Any
    ) -> List[BuildStep]:
            ...
            try:
                ...
                if step.type == "workbook_visualization" and step.config:
                    instruction = VisualizationInstruction(
                        ...
                        visualization_type=step.config.get(
                            "visualization_type", "chart"
                        ),
                        data_source=step.config.get("data_source", "default"),
                        chart_config={
                            "type": step.config.get("visualization_type", "chart")
                        },
                        layout_instructions={"position": "auto", "size": "medium"},
                    )
                    result = (
                        await self.workbook_service.push_visualization_instructions(
                            instruction
                        )
                    )
                    step.result = result
    ...
    async def _test_build(
        self, results: List[BuildStep], branch: Any
    ) -> Dict[str, Any]:
