...
class AIPModelClient:
    ...
    def __init__(self, foundry_client):
        ...
        self.base_url = getattr(
            foundry_client, "foundry_url", "https://raiderexpress.palantirfoundry.com"
        )
    ...
    async def create_completion(
        ...
        try:
            if FOUNDRY_V2_AVAILABLE and hasattr(self.foundry_client, "aip_agents"):
                result = await self._sdk_completion(
                    prompt, model, max_tokens, temperature, context
                )
            else:
                result = await self._http_completion(
                    prompt, model, max_tokens, temperature, context
                )
        ...
        try:
            async with await connection_pool.get_client() as client:
                ...
                if response.status_code == 200:
                    ...
                    self.logger.error(
                        "embedding_api_error", status=response.status_code
                    )
                    return []
...
Response:"""
        ...
        result = await self.aip_client.create_completion(
            prompt=prompt, context=context, max_tokens=1500, temperature=0.7
        )
