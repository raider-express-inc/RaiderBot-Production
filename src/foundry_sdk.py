...
class FoundryClient:
    ...
    def __init__(
        self, auth_token=None, foundry_url=None, client_id=None, client_secret=None
    ):
        self.auth_token = auth_token or os.getenv("FOUNDRY_TOKEN")
        self.foundry_url = foundry_url or os.getenv(
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                response = await client.get(
                    f"{self.foundry_url}/api/discovery/workshop", headers=self.headers
                )
    ...
    async def create_workshop_app(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
                ...
                for endpoint in endpoints_to_try:
                    ...
                    if response.status_code in [200, 201]:
                        try:
                            ...
                            app_id = response_data.get(
                                "id",
                                response_data.get(
                                    "rid", f"workshop_{datetime.now().timestamp()}"
                                ),
                            )
                        except Exception:
                            response_data = {
                ...
                raise Exception(
                    f"All Workshop API endpoints failed. Attempted: {endpoints_to_try}"
                )
        except Exception as e:
    ...
    async def update_workbook_visualization(
        self, workbook_id: str, viz_config: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
                ...
                for endpoint in endpoints_to_try:
                    ...
                    response = await client.post(
                        url, headers=self.headers, json=workshop_config
                    )

                    if response.status_code in [200, 201]:
                        try:
                            api_response = response.json()
                            viz_id = api_response.get(
                                "id",
                                api_response.get(
                                    "widget_id", f"viz_{datetime.now().timestamp()}"
                                ),
                            )
                        except Exception:
                            api_response = {
                ...
                raise Exception(
                    f"All visualization update endpoints failed. Attempted: {endpoints_to_try}"
                )
        except Exception as e:
    ...
    async def create_user_dashboard(
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0, follow_redirects=True) as client:
                ...
                workshop_app_config = {
                    "name": dashboard_config.get(
                        "name", f"RaiderBot Dashboard - {dashboard_config['user_id']}"
                    ),
                ...
                for endpoint in endpoints_to_try:
                    url = f"{self.foundry_url}{endpoint}"
                    response = await client.post(
                        url, headers=self.headers, json=workshop_app_config
                    )
                    ...
                    if response.status_code in [200, 201]:
                        try:
                            ...
                            app_id = response_data.get(
                                ...
                                response_data.get(
                                    "rid",
                                    f"dashboard_{dashboard_config['user_id']}_{datetime.now().timestamp()}",
                                ),
                            )
                        except Exception:
                            response_data = {
                ...
                raise Exception(
                    f"All dashboard creation endpoints failed. Attempted: {endpoints_to_try}"
                )
        except Exception as e:
                    ...
                    if response.status_code == 200:
                        try:
                            ...
                            if isinstance(apps_data, dict):
                                apps_list = apps_data.get(
                                    "applications", apps_data.get("data", [apps_data])
                                )
                            else:
                                apps_list = apps_data
                                ...
                                workbooks.append(
                                    {
                                        "workbook_id": app.get(
                                            ...
                                            app.get(
                                                "rid",
                                                f"workshop_{user_id}_{len(workbooks)}",
                                            ),
                                        ),
                                        "name": app.get(
                                            "name", f"Workshop App {len(workbooks) + 1}"
                                        ),
                                        "type": "workshop_application",
