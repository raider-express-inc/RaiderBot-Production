...
class MCPSnowflakeIntegration:
                ...
                try:
                    with open(config_path, "r") as f:
                        ...
                        if "mcpServers" in config:
                            ...
                            "mcp-config.json"
                        ) and "foundry-integration" in str(config):
                            self.mcp_servers["foundry-integration"] = config
                            logger.info(
                                f"✅ Loaded Foundry MCP server from {config_path}"
                            )
                except Exception as e:
                    logger.error(
                        f"❌ Failed to load MCP config from {config_path}: {e}"
                    )
    ...
    def check_zapier_availability(self):
        ...
        try:
            ...
            if importlib.util.find_spec("mcp_tool_call"):
                zapier_servers = [
                    name for name in self.mcp_servers.keys() if "zapier" in name.lower()
                ]
                if zapier_servers:
    ...
    def execute_with_mcp_integration(
        self, query: str, mcp_context: Optional[Dict] = None
    ) -> Dict[str, Any]:
    ...
    def _enhance_with_mcp_tools(
        self, data: List[Dict], mcp_context: Dict
    ) -> List[Dict]:
        ...
        try:
            ...
            if "foundry-integration" in self.mcp_servers and mcp_context.get(
                "foundry_sync"
            ):
    ...
    def natural_language_query_with_automation(
        self, question: str, automation_config: Optional[Dict] = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["success"] and automation_config:
                if automation_config.get("zapier_webhook") and self.zapier_available:
                    self._trigger_zapier_webhook(
                        result["data"], automation_config["zapier_webhook"]
                    )

                if automation_config.get("foundry_update"):
                    self._update_foundry_dashboard(
                        result["data"], automation_config["foundry_update"]
                    )
    ...
    def _trigger_zapier_webhook(self, data: Any, webhook_config: Dict):
        ...
        try:
            logger.info(
                f"🔗 Would trigger Zapier webhook: {webhook_config.get('url', 'configured')}"
            )
        except Exception as e:
    ...
    def _update_foundry_dashboard(self, data: Any, dashboard_config: Dict):
        ...
        try:
            logger.info(
                f"🏗️ Would update Foundry dashboard: {dashboard_config.get('dashboard_id', 'configured')}"
            )
        except Exception as e:
