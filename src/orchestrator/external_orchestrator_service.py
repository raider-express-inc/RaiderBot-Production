...
class ExternalOrchestratorService:
    ...
    async def register_agent(
        self, agent_type: AgentType, agent_config: Dict[str, Any]
    ) -> str:
    ...
    def _generate_summary(self, results: List[Dict[str, Any]]) -> str:
        ...
        completed_agents = [
            r["agent_type"] for r in results if r["status"] == "completed"
        ]
        return (
            f"Coordinated {len(completed_agents)} agents: {', '.join(completed_agents)}"
        )
