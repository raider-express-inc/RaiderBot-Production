...
class MCPIntegration:
    ...
    async def _execute_snowflake_tool(
    ...
    async def _execute_foundry_tool(
        self, tool: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_multi_system_tool(
        self, tool: str, parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def deploy_mcp_servers(self, foundry_client) -> Dict[str, Any]:
        ...
        for server_name, server_config in self.servers.items():
            try:
                ...
                deployment_results.append(
                    {
                        ...
                        "deployment_id": result.get(
                            "deployment_id", f"{server_name}_001"
                        ),
                    }
                )
                ...
                self.logger.error(
                    f"MCP server deployment failed for {server_name}: {str(e)}"
                )
                deployment_results.append(
                    {"server": server_name, "status": "failed", "error": str(e)}
                )
