...
def investigate_table_structure():
    ...
    try:
        ...
        try:
            columns = cortex_client.execute_query(
                "DESCRIBE TABLE SQL_SERVER_DBO.ORDERS"
            )
        ...
        try:
            sample = cortex_client.execute_query(
                "SELECT * FROM SQL_SERVER_DBO.ORDERS LIMIT 3"
            )
            if sample:
        ...
        for table in order_tables:
            try:
                columns = cortex_client.execute_query(
                    f"DESCRIBE TABLE SQL_SERVER_DBO.{table}"
                )
