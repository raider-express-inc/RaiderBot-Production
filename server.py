...
@app.tool()
def search_orders_with_automation(
    query: str, filters: Optional[Dict] = None, automation: Optional[Dict] = None
) -> Dict[str, Any]:
    """Search orders with optional MCP automation triggers"""
    try:
        result = mcp_integration.execute_with_mcp_integration(
            f'SELECT * FROM "dbo"."orders" WHERE {query}', mcp_context=automation
        )
        return result
    except Exception as e:
...
def search_orders(query: str, filters: Optional[Dict] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if "TMS" in query.upper() and (
            "VS" in query.upper() or "VERSUS" in query.upper()
        ):
            sql_query = """
            SELECT 
                COMPANY_ID,
        ...
        if "TMS" in query.upper() and (
            ...
            for row in results:
                company_name = (
                    ...
                    if row["COMPANY_ID"] == "TMS"
                    else "Raider Logistics (Brokerage)"
                )
...
def revenue_summary(timeframe: str = "week") -> Dict[str, Any]:
    ...
    try:
        ...
        total_orders = sum(
            int(row["ORDER_COUNT"]) if row["ORDER_COUNT"] else 0 for row in results
        )
            ...
            formatted_results.append(
                {
                    ...
                    "earliest_order": str(row["EARLIEST_ORDER"])
                    if row["EARLIEST_ORDER"]
                    else None,
                    "latest_order": str(row["LATEST_ORDER"])
                    if row["LATEST_ORDER"]
                    else None,
                }
            )
...
def analyze_customer(
    analysis_type: str = "top_customers", limit: int = 10
) -> Dict[str, Any]:
...
def build_this_out(request: str, user_id: str = "default_user") -> Dict[str, Any]:
    ...
    try:
        ...
        if bot_integration and any(
            cmd in request.lower() for cmd in bot_integration.command_mappings.keys()
        ):
            import asyncio
            ...
            command = next(
                (
                    cmd
                    for cmd in bot_integration.command_mappings.keys()
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor() as executor:
                    future = executor.submit(
                        asyncio.run,
                        bot_integration.process_bot_command(command, user_id),
                    )
        ...
        build_request = BuildRequest(
            id=str(uuid.uuid4()), user_id=user_id, natural_language_request=request
        )
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor() as executor:
                future = executor.submit(
                    asyncio.run, foundry_engine.process_build_request(build_request)
                )
                result = future.result()
        except RuntimeError:
...
if __name__ == "__main__":
    ...
    try:
        test_result = snowflake_client.execute_query(
