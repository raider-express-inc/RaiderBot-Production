    ...
    try:
        ...
        try:
            ...
            for col in structure:
                ...
                is_business = any(
                    pattern in col_name.upper() for pattern in key_patterns
                )
                if is_business:
        ...
        try:
            if business_columns:
                col_names = [
                    col[0] for col in business_columns[:10]
                ]  # First 10 business columns
        ...
        if business_columns:
            ...
            customer_cols = [
                col
                for col, _ in business_columns
                if any(x in col.upper() for x in ["CUSTOMER", "COMPANY", "CLIENT"])
            ]
            amount_cols = [
                col
                for col, _ in business_columns
                if any(
                    x in col.upper()
                    for x in ["AMOUNT", "TOTAL", "CHARGE", "COST", "PRICE"]
                )
            ]
            status_cols = [
                col for col, _ in business_columns if "STATUS" in col.upper()
            ]
