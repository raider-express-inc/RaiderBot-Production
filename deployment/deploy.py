...
class FoundryDeployer:
    def __init__(self):
        ...
        self.foundry_url = self.config.get(
    ...
    async def deploy_automation(self):
        ...
        print(
            f"🌐 Access RaiderBot through Foundry Workshop at: {self.foundry_url}/workspace/raiderbot"
        )
        print(
            f"🎯 Workshop Build Console: {self.foundry_url}/workspace/raiderbot/workshop/build-console"
        )
        ...
        print(
            "🦸‍♂️ Users can now access connected dashboards with visualization instructions!"
        )
    ...
    def _verify_connection(self) -> bool:
        ...
        try:
            ...
            return bool(
                self.config.get("FOUNDRY_CLIENT_ID")
                or self.config.get("FOUNDRY_AUTH_TOKEN")
            )
    ...
    async def _create_aip_agent(self) -> Optional[str]:
        ...
        try:
            ...
            if result.get("status") == "created":
                ...
                raise Exception(
                    f"Agent creation failed: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
    ...
    async def _create_workshop_app(self) -> Optional[str]:
        ...
        try:
            ...
            if result.get("status") == "created":
                ...
                raise Exception(
                    f"Workshop app creation failed: {result.get('error', 'Unknown error')}"
                )
        except Exception as e:
    ...
    async def _provision_user_dashboards(self):
        ...
        for user in users:
            try:
                ...
                result = await self.foundry_client.create_user_dashboard(
                    dashboard_config
                )
                if result.get("status") in ["created", "updated"]:
                    ...
                    print(
                        f"  ❌ {user['name']} ({user['role']}): Dashboard creation failed - {result.get('error', 'Unknown error')}"
                    )
...
async def main():
    ...
    if not (
        deployer.config.get("FOUNDRY_CLIENT_ID")
        or deployer.config.get("FOUNDRY_AUTH_TOKEN")
    ):
