...
class ZapierMCPServer:
    ...
    async def fetch_zap_data(self, zap_id: str, limit: int = 10) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": str(e), "zap_id": zap_id}
    ...
    async def list_available_zaps(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(
                "https://zapier.com/api/v1/zaps", headers=headers, timeout=30
            )
            response.raise_for_status()
    ...
    return [
        ...
        types.Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {
                    "zap_id": {"type": "string", "description": "Zapier automation ID"},
                    "limit": {
                        "type": "integer",
    ...
    try:
        if name == "trigger_zapier_webhook":
            result = await zapier_server.trigger_webhook(
                arguments["webhook_id"], arguments["data"]
            )
        elif name == "fetch_zap_data":
            result = await zapier_server.fetch_zap_data(
                arguments["zap_id"], arguments.get("limit", 10)
            )
